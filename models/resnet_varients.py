    ...
    **kwargs,
    ...
    for stage_idx, (planes, num_blocks, db) in enumerate(
        ...
        if stride != 1 or inplanes != planes * block_fn.expansion:
            ...
            downsample = (
                downsample_avg(**down_kwargs) if avg_down else downsample_conv(**down_kwargs)
            )
        ...
        for block_idx in range(num_blocks):
            ...
            new_block_kwargs = deepcopy(block_kwargs)
            # use se in layer{5-num_selayers} - layer4
            if stage_idx < 4 - num_selayers:
                new_block_kwargs.pop("attn_layer")
...
def seresnet18(pretrained=False, **kwargs):
    model_args = dict(
        block=BasicBlock, layers=[2, 2, 2, 2], block_args=dict(attn_layer="se"), **kwargs
    )
...
def seresnet34(pretrained=False, **kwargs):
    model_args = dict(
        block=BasicBlock, layers=[3, 4, 6, 3], block_args=dict(attn_layer="se"), **kwargs
    )
...
def seresnet50(pretrained=False, **kwargs):
    model_args = dict(
        block=Bottleneck, layers=[3, 4, 6, 3], block_args=dict(attn_layer="se"), **kwargs
    )
...
def seresnet101(pretrained=False, **kwargs):
    model_args = dict(
        block=Bottleneck, layers=[3, 4, 23, 3], block_args=dict(attn_layer="se"), **kwargs
    )
...
def seresnet152(pretrained=False, **kwargs):
    model_args = dict(
        block=Bottleneck, layers=[3, 8, 36, 3], block_args=dict(attn_layer="se"), **kwargs
    )
