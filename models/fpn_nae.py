...
class NAEWithFPN(nn.Module):
    def __init__(
        ...
        super(NAEWithFPN, self).__init__()
        ...
        if rpn_anchor_generator is None:
            ...
            if anchor_ratios is None:
                anchor_ratios = ((0.5, 1.0, 2.0),) * len(anchor_scales)
        ...
        if box_predictor is None:
            representation_size = 1024
            box_predictor = CoordRegressor(representation_size, num_classes=2, RCNN_bbox_bn=True)

        if embedding_head is None:
            embedding_head = NormAwareEmbeddingProj(in_channel=1024, dim=256)
...
class NormAwareRoIHeads(RoIHeads):
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        box_regression = self.box_predictor(rcnn_features)
...
class NormAwareEmbeddingProj(nn.Module):
    def __init__(self, in_channel=1024, dim=256):
        super(NormAwareEmbeddingProj, self).__init__()
        self.in_channel = in_channel
        self.dim = dim

        proj = nn.Sequential(nn.Linear(in_channel, dim), nn.BatchNorm1d(dim))
...
def build_fpn_nae_model(cfg, pretrained_backbone=True):
    backbone = resnet_fpn_backbone("resnet50", pretrained_backbone, trainable_layers=4)
    model = NAEWithFPN(
        backbone,
        # box_head=conv_head,
